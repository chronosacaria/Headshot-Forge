...
public class DealHeadshotEvent {
    ...
    public static void onHeadshotIfApplicable(LivingDamageEvent event){
        ...
        if (event.getSource().isProjectile()){
            ...
            double headStart =
                    entity.getPositionVec().add(0.0, entity.getSize(entity.getPose()).height * 0.85, 0.0).y - 0.17;
            if (!ignore && event.getSource().getDamageLocation().y > headStart && event.getSource() != null && !((LivingEntity)entity).canBlockDamageSource(event.getSource())){
